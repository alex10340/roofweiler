		...
		switch (section.type) {
			case "paragraph":
				return (
					<p
						key={index}
						className="text-lg leading-relaxed text-[#a0a09f] mb-6"
					>
						{section.content}
					</p>
				);
			...
			case "heading":
				return (
					<h3
						key={index}
						className="text-2xl font-semibold mb-4 text-[#ffffff]"
					>
						{section.content}
					</h3>
				);
					...
					<div key={index} className="mb-6">
						...
						<ul className="space-y-4">
							{section.items?.map((item, i) => (
								<li
									key={i}
									className="text-lg leading-relaxed text-[#a0a09f] flex items-start gap-3"
								>
									<span className="text-[#ffde11] mt-1">•</span>
									<span>
										{item.title && (
											<strong className="text-[#ffffff]">{item.title}:</strong>
										)}{" "}
										{item.content}
									</span>
								</li>
